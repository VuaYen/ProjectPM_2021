...
public interface OrderRepository extends JpaRepository<ScheduledDelivery,Integer> {
    ...
    @Query(value="select o.* " +
            ...
            " where o.id=oi.scheduled_delivery_id and oi.detials_id=i.id and o.vendor_id=v.id " +
    ...
    @Query(value="SELECT count(distinct o.id), c.name " +
            "     FROM orders  o, orders_items oi, orderdetail i, products p, vendor v, categories c " +
            "     WHERE o.id=oi.order_id and oi.detials_id=i.id and i.product_id=p.id and p.vendor_id=v.id and p.category_id=c.id " +
            "     GROUP BY c.id" +
    ...
    @Query(value="SELECT count(distinct o.id), v.business_name " +
            "     FROM orders  o, orders_items oi, orderdetail i, products p, vendor v " +
            "     WHERE o.id=oi.order_id and oi.detials_id=i.id and i.product_id=p.id and p.vendor_id=v.id " +
            "     GROUP BY v.id" +
            "     ORDER BY count(distinct o.id) DESC " +
            "     LIMIT 10",nativeQuery=true)
    ...
    @Query(value="SELECT count(distinct o.id), p.name " +
            "     FROM orders  o, orders_items oi, orderdetail i, products p " +
            "     WHERE o.id=oi.order_id and oi.detials_id=i.id and i.product_id=p.id" +
            "     GROUP BY p.id " +
            "     ORDER BY count(distinct o.id) DESC " +
            "     LIMIT 10",nativeQuery=true)
    ...
    @Query(value="SELECT count(distinct o.id) " +
            "     FROM orders  o, orders_items oi, orderdetail i, products p, vendor v " +
            "     WHERE o.id=oi.order_id and oi.detials_id=i.id and i.product_id=p.id and p.vendor_id=v.id and v.id= :vendorId " +
            "     GROUP BY v.id "
            ,nativeQuery=true)
    ...
    @Query(value="SELECT count(distinct o.id), c.name " +
            "     FROM orders  o, orders_items oi, orderdetail i, products p, vendor v, categories c " +
            "     WHERE o.id=oi.order_id and oi.detials_id=i.id and i.product_id=p.id and p.category_id=c.id and p.vendor_id=v.id and v.id= :vendorId " +
            "     GROUP BY c.id " +
            "     ORDER BY count(distinct o.id) DESC " +
            "     LIMIT 10 "
            ,nativeQuery=true)
    ...
    @Query(value="SELECT count(distinct o.id),a.state " +
            "     FROM orders o, addresses a, orders_items oi, orderdetail i, products p, vendor v " +
            "     WHERE o.shipping_address_id=a.id and o.id=oi.order_id and oi.detials_id=i.id and i.product_id=p.id and p.vendor_id=v.id and v.id=:vendorId " +
